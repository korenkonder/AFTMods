...
#define SHADER_FLAGS_ANISO_TANGENT            (bitfieldExtract(g_shader_flags.x, 2, 2)) // bit 2:3
#define SHADER_FLAGS_COMPOSITE_BACK           (bitfieldExtract(g_shader_flags.x, 4, 1)) // bit 4
#define SHADER_FLAGS_BLEND_FUNC_01            (bitfieldExtract(g_shader_flags.x, 5, 3)) // bit 5:7
...
#define SHADER_FLAGS_DOUBLE_SIDE              (bitfieldExtract(g_shader_flags.x, 15, 1)) // bit 15
...
#define SHADER_FLAGS_FADE_COLOR               (bitfieldExtract(g_shader_flags.x, 24, 1)) // bit 24
#define SHADER_FLAGS_FADE_TYPE                (bitfieldExtract(g_shader_flags.x, 25, 3)) // bit 25:27
...
#define SHADER_FLAGS_FOG                      (bitfieldExtract(g_shader_flags.y, 0, 2)) // bit 32+0:1
#define SHADER_FLAGS_FOGMAP                   (bitfieldExtract(g_shader_flags.y, 2, 2)) // bit 32+2:3
...
#define SHADER_FLAGS_NUM_SHADOWMAP            (bitfieldExtract(g_shader_flags.z, 1, 2)) // bit 64+1:2
#define SHADER_FLAGS_REFLECT                  (bitfieldExtract(g_shader_flags.z, 3, 2)) // bit 64+3:4
#define SHADER_FLAGS_REDUCE_TEX               (bitfieldExtract(g_shader_flags.z, 5, 4)) // bit 64+5:8
#define SHADER_FLAGS_SELF_SHADOW              (bitfieldExtract(g_shader_flags.z, 9, 1)) // bit 64+9
#define SHADER_FLAGS_SHADOW_SAMPLE            (bitfieldExtract(g_shader_flags.z, 10, 2)) // bit 64+10:11
#define SHADER_FLAGS_U2D                      (bitfieldExtract(g_shader_flags.z, 12, 3)) // bit 64+12:14
#define SHADER_FLAGS_SHOW_MIPMAP              (bitfieldExtract(g_shader_flags.z, 15, 2)) // bit 64+15:16
...
#define SHADER_FLAGS_SNOW_TYPE                (bitfieldExtract(g_shader_flags.z, 20, 2)) // bit 64+20:21
#define SHADER_FLAGS_SPECULAR                 (bitfieldExtract(g_shader_flags.z, 22, 2)) // bit 64+22:23
...
#define SHADER_FLAGS_TEX_COLOR                (bitfieldExtract(g_shader_flags.w, 2, 2)) // bit 96+2:3
#define SHADER_FLAGS_TEX_ENVMAP               (bitfieldExtract(g_shader_flags.w, 4, 1)) // bit 96+4
#define SHADER_FLAGS_TEX_FORMAT               (bitfieldExtract(g_shader_flags.w, 5, 2)) // bit 96+5:6
#define SHADER_FLAGS_TEX_LUCENCY              (bitfieldExtract(g_shader_flags.w, 7, 1)) // bit 96+7
#define SHADER_FLAGS_TEX_NORMAL               (bitfieldExtract(g_shader_flags.w, 8, 1)) // bit 96+8
#define SHADER_FLAGS_TEX_PARENCY              (bitfieldExtract(g_shader_flags.w, 9, 1)) // bit 96+9
#define SHADER_FLAGS_TEX_REFLECTMAP           (bitfieldExtract(g_shader_flags.w, 10, 1)) // bit 96+10
#define SHADER_FLAGS_TEX_REFRACTMAP           (bitfieldExtract(g_shader_flags.w, 11, 1)) // bit 96+11
#define SHADER_FLAGS_TEX_REFRACTMAP_WATERRING (bitfieldExtract(g_shader_flags.w, 12, 1)) // bit 96+12
#define SHADER_FLAGS_TEX_SHADOW               (bitfieldExtract(g_shader_flags.w, 13, 1)) // bit 96+13
#define SHADER_FLAGS_TEX_SPECULAR             (bitfieldExtract(g_shader_flags.w, 14, 1)) // bit 96+14
#define SHADER_FLAGS_TONE_MAP_METHOD          (bitfieldExtract(g_shader_flags.w, 15, 2)) // bit 96+15
#define SHADER_FLAGS_UV_LAYER                 (bitfieldExtract(g_shader_flags.w, 16, 1)) // bit 96+16
