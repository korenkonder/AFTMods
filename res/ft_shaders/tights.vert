...
layout(location = 4) out vec4 result_texcoord_shadow0; //xyz=texcoord_shadow0, w=fog
layout(location = 5) out vec3 result_eye;
layout(location = 6) out vec4 result_face_offset; //x=, yz=unused, w=face_offset
layout(location = 7) out vec4 result_fog_color; //xyz=fog_color, w=fresnel
...
void main() {
    ...
    vec3 tangent_w;
        ...
        vec3 normal0 = apply_skinning(a_normal, mtxidx[0]);
        vec3 normal1 = apply_skinning(a_normal, mtxidx[1]);
        if (mtxidx[1] < 0.0)
            normal1 = normal0;

        vec3 normal = cross(normal0, normal1);
        result_face_offset.x = clamp(dot(normal, normal), 0.0, 1.0);
    #else
        pos_m = vec4(a_position, 1.0);

        normal_w = a_normal * mat3(g_worlds);
        tangent_w = a_tangent.xyz * mat3(g_worlds);

        result_face_offset.x = 0.0;
    #endif
    ...
    result_face_offset.yz = vec2(0.0);

    result_fog_color.w = get_fresnel(normal_w, eye_w, 8.0, g_fresnel_coefficients.z);
