...
layout(location = 2) in vec4 frg_texcoord2; //xy=texcoord4, zw=texcoord5
layout(location = 3) in vec4 frg_texcoord3; //xy=texcoord6, zw=texcoord7
...
void main() {
    ...
    sum += textureLod(g_texture, frg_texcoord2.xy, g_texture_lod.x);
    sum += textureLod(g_texture, frg_texcoord2.zw, g_texture_lod.x);
    sum += textureLod(g_texture, frg_texcoord3.xy, g_texture_lod.x);
    sum += textureLod(g_texture, frg_texcoord3.zw, g_texture_lod.x);
