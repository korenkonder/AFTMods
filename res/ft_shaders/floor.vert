...
layout(location = 2) out vec4 result_texcoord_shadow0; //xyz=texcoord_shadow0, w=fog
layout(location = 3) out vec4 result_texcoord_shadow1; //xyz=texcoord_shadow1, w=
layout(location = 4) out vec4 result_color;
layout(location = 5) out vec4 result_spec_color;
...
void main() {
    ...
    float fres = pow(clamp(1.0 - dot(normal_w, eye_w), 0.0, 1.0), 5.0);
    ...
    diff += g_light_stage_diff * lc.y;

    vec4 spec;
    spec.rgb = g_light_stage_spec.rgb * lc.z;
    spec.w = 1.0;
    result_spec_color = spec * spec_ratio;
