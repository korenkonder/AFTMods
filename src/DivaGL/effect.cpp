...
struct EffectStorage {
    ...
    inline void unmap_buffer(p_gl_rend_state& p_gl_rend_st) {
        if (DIVA_GL_VERSION_4_3)
            p_gl_rend_st.unmap_array_buffer(ssbo);
        else
...
void snow_particle_draw(render_data_context& rend_data_ctx, const cam_data& cam) {
    ...
    rend_data_ctx.state.active_bind_texture_2d(0, tex->glid);
    rend_data_ctx.state.active_bind_texture_2d(1, render_get()->rend_texture[0].get_depth_texture_glid());
    rend_data_ctx.state.bind_vertex_array(rctx->common_vao);
...
HOOK(void, FASTCALL, snow_particle_ctrl, 0x000000014035CA20) {
    ...
    snow_particle_vertex_data* fallen_vtx_data
        = (snow_particle_vertex_data*)snow_fallen_storage.map_buffer(gl_state);
    if (fallen_vtx_data) {
        particle_data* snow_ptcl_fallen = snow_ptcl_fallen_data;
...
void EffectFogRing::draw(render_data_context& rend_data_ctx, const cam_data& cam) {
    ...
    rend_data_ctx.state.set_viewport(0, 0,
        rt.m_txhd_color->get_width_align_mip_level(),
        rt.m_txhd_color->get_height_align_mip_level());
    ...
    render_manager.set_effect_texture(rt.get_texture());
    gl_get_error_print();
}
...
void EffectRipple::copy_to_ripple_tex(render_data_context& rend_data_ctx, RenderTexture* rt) {
    ...
    field_BB8.attach_texture(ripple_tex->glid);
    field_BB8.begin_render(rend_data_ctx.state);
    image_filter_scale(rend_data_ctx, &field_BB8, rt->get_texture());
    field_BB8.end_render(rend_data_ctx.state);
}
...
void EffectRipple::draw(render_data_context& rend_data_ctx, const cam_data& cam) {
    ...
    render_manager.set_effect_texture(
        render_manager.get_render_texture(v11).get_texture());
...
void star_catalog::draw(render_data_context& rend_data_ctx, const cam_data& cam) {
    ...
    scene_shader_data.g_transform[2] = temp.row2;
    scene_shader_data.g_transform[3] = temp.row3;
    rend_data_ctx.state.write_uniform_buffer(stars_scene_ubo, scene_shader_data);
    ...
    for (int32_t i = 0; i < 2; i++) {
        ...
        rend_data_ctx.state.write_uniform_buffer(stars_batch_ubo, batch_shader_data);
...
void water_particle::draw(render_data_context& rend_data_ctx, const cam_data& cam, mat4* mat) {
    ...
    water_particle_vertex_data* vtx_data = (water_particle_vertex_data*)storage.map_buffer(rend_data_ctx.state);
    if (!vtx_data)
        return;
    ...
    storage.unmap_buffer(rend_data_ctx.state);
...
static void draw_fog_particle(render_data_context& rend_data_ctx,
    ...
    rend_data_ctx.state.bind_vertex_array(rctx->common_vao);
    storage.Bind(rend_data_ctx.state, 0);
    storage.Draw(rend_data_ctx.state, GL_TRIANGLES, 0, data->num_vtx);
    rend_data_ctx.state.bind_vertex_array(0);
    rend_data_ctx.state.disable_blend();
    shader::unbind(rend_data_ctx.state);
}
...
static void ripple_propagate(render_data_context& rend_data_ctx,
    ...
    if (dst->m_txhd_color->internal_format == GL_RGBA32F
        || dst->m_txhd_color->internal_format == GL_RGBA16F)
